...
//			System.out.println(texts);
			...
			for(Object o:set) {
				...
				if(o.toString().startsWith("enemy_")) {
	                imgMap.put(o.toString(), new ImageIcon(url));
	            }
				else if(o.toString().startsWith("player2_")) {
					imgMap.put(o.toString(), new ImageIcon(url));
				}
				else {
	            	imgMap.put(o.toString(), new ImageIcon(url));
	...
	public static void loadPlay(int playerCount) {
		loadObj();
		String playStr1="500,500,up";
		ElementObj obj1=getObj("play");  
		ElementObj play1 = obj1.createElement(playStr1);
		if(play1 instanceof Play) {
			((Play)play1).setPlayer1(true);
		}
		em.addElement(play1, GameElement.PLAY);
		
		if(playerCount==2) {
			String playStr2="700,500,up";
			ElementObj obj2 = getObj("play");  
	        ElementObj play2 = obj2.createElement(playStr2);
	        if(play2 instanceof Play) {
				((Play)play2).setPlayer1(false);
			}
	 ...
	 * 加载敌人
	 * @param count 敌人数量
	 * @param initParams 初始参数格式（如："x,y,direction"）
	 */
	public static void loadEnemy(int count) {
	    loadObj();
	    ...
	    for (int i = 0; i < count; i++) {
	        ...
	        if (enemyObj != null) {
	            ElementObj enemy = enemyObj.createElement(x + "," + y + "," + dir);
