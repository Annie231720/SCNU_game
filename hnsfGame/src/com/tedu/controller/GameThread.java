...
//		加载主角
		GameLoad.loadPlay(1);//也可以带参数，单机还是2人
//		加载敌人NPC等
		GameLoad.loadEnemy(1);//加载敌人，敌人生成个数
...
//		全部加载完成，游戏启动
	...
	private void gameRun() {
		...
		while(true) {
			...
			List<ElementObj> player1=em.getElementsByKey(GameElement.PLAY);
			...
			ElementPK(enemys,files);//敌人与玩家子弹的碰撞检测
			ElementPK(files,maps);//子弹和地图的碰撞检测
			ElementPK(player1 , enemys);
			ElementPK(enemyfiles, maps);
			ElementPK(enemyfiles, player1);
...
//					扩展 留给大家
	...
	private void checkGameState() {
	    ...
	    // 玩家全部死亡
	    if (players.isEmpty()) {
	        em.setGameState(GameStateElement.LOSE);
	    }
	    // 敌人全部被消灭
	    else if (enemies.isEmpty()) {
	        em.setGameState(GameStateElement.WIN);
	    }
	}	
