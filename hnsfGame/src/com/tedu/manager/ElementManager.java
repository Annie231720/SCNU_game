...
//		道具，子弹，爆炸效果，死亡效果。。。。
		 GameStateElement stateElement = new GameStateElement();
		  addElement(stateElement, GameElement.GAME_STATE);
	}
	// 添加获取游戏状态的方法
	public int getGameState() {
	    List<ElementObj> stateList = getElementsByKey(GameElement.GAME_STATE);
	    if (!stateList.isEmpty()) {
	        GameStateElement state = (GameStateElement) stateList.get(0);
	        return state.getState();
	    }
	    return GameStateElement.RUNNING;
	}
	...
	public void setGameState(int state) {
	    List<ElementObj> stateList = getElementsByKey(GameElement.GAME_STATE);
	    if (!stateList.isEmpty()) {
	        GameStateElement stateElement = (GameStateElement) stateList.get(0);
